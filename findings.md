...
=======
- The base mechanism now targets imported level18 `GameSceneDualQueue2` data rather than the previous level12-style layout or original six-vehicle prototype.
- Active data has 47 vehicles, two fixed queues with 115 and 191 groups, and 306 total passenger groups.
- Vehicle seat totals match fixed passenger queue totals by color: 0=38, 1=50, 2=10, 3=62, 4=46, 5=56, 8=44.
- CSV front-vehicle relationships are the active `vehicleDepthes` source for level18. There are 42 blocker entries, and initial movable vehicles are `30, 31, 32, 56, 58`.
- The active level includes two garage containers (`1`, `2`) and 16 stocked garage vehicles with `containerType: 2`.
>>>>>>> Stashed changes
