...
| Upgrade-spot and double-gate capacity rules for the first parking spot | `src/mechanics/upgrade-spot/model.js`, `src/mechanics/double-gate/model.js` | `src/game-model.js`, `src/scene-view.js`, `test/capacity-mechanics.test.js`, `test/mechanic-registry.test.js` |
...
- `src/mechanics/index.js`: assembles and freezes the 18-module catalog (11 playable and 7 planned), then owns exact module lookup, playable-runtime resolution, composite runtime hooks, and optional generic detail-view factory delegation.
...
- `src/mechanics/upgrade-spot/`: playable upgrade-spot mechanic. `model.js` doubles the effective passenger-group capacity for vehicles assigned to the first parking spot while preserving original vehicle model sizing.
- `src/mechanics/double-gate/`: playable double-gate mechanic. `model.js` keeps vehicle capacity unchanged but makes each passenger group boarding through the first parking spot consume two capacity groups.
- `src/mechanics/maglev-spot/`: playable maglev-spot mechanic. `model.js` owns the fixed maglev vehicle set, global raised/lowered state toggled after successful dispatches, dispatch blocking while raised, and ground-blocker filtering.
